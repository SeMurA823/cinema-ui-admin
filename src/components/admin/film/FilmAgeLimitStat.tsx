...
export default function FilmAgeLimitStat(props: Props) {
    ...
    useEffect(() => {
        const asyncFoo = async () => {
            try {
                const response = await $api.get<LimitCountry[]>((`/stat/filmlimits?start=${encodeURIComponent(props.start.toISOString(false))}&end=${encodeURIComponent(props.end.toISOString(false))}`));
